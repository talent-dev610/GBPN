...
import 'package:gbpn_messages/screen/home/chat/chat_screen.dart';
...
class _MessageListScreenState extends State<MessageListScreen> {
  ...
  void initState() {
    ...
    if (conversationState is ConversationLoadedState) {
      _conversations = Utilities.filterByType(conversationState.conversations, ConversationType.SMS);
    }
  }

  @override
  Widget build(BuildContext context) {
    List<ConversationCollectionModel> collections = Utilities.sortConversationsByPhone(_conversations);
    return BlocListener<ConversationBloc, ConversationState>(
        listener: (context, state) {
          if (state is ConversationLoadedState) {
            ...
            setState(() {
              _conversations = Utilities.filterByType(state.conversations, ConversationType.SMS);
            });
          }
        },
      child: Scaffold(
        ...
        body: SmartRefresher(
            ...
            child: collections.isEmpty ? buildEmpty() : ListView.separated(
                ...
                itemBuilder: (ctx, idx) {
                  ...
                  ConversationItemModel lastConversation = collection.conversations.last;
                  return Dismissible(
                      key: UniqueKey(),
                      direction: DismissDirection.horizontal,
                      confirmDismiss: (_) async {
                        AppBloc.conversationBloc
                            .add(ConversationDeleteEvent(conversation: lastConversation));
                        return true;
                      },
                      background: Align(
                      ...
                      child: Material(
                        ...
                        child: InkWell(
                          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => ChatScreen(phone: collection.phone),)),
                                ...
                                children: [
                                  ...
                                  Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text(collection.phone.replaceAll('+', '').replaceAllMapped(RegExp(r'1(\d{3})(\d{3})(\d+)'), (Match m) => "(${m[1]}) ${m[2]}-${m[3]}"), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                                      ...
                                      if (collection.getUnreadMessageCount() > 0)
                                        Padding(
                                          ...
                                          child: Container(
                                            ...
                                            child: Center(child: Text(collection.getUnreadMessageCount().toString(), style: const TextStyle(color: Colors.white, fontSize: 10),),),
                                          ),
                                        ),
                                      Text(Utilities.formatDateTimeMessage(Utilities.convertToLocalTime(collection.conversations.last.createdAt!), onlyTime: false, onlyDate: true, inline: false), textAlign: TextAlign.right, style: const TextStyle(fontSize: 12, color: Colors.white))
                                    ],
                                  ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text("No messages", style: TextStyle(color: kGray.withOpacity(0.5), fontSize: 18),)
        ],
      ),
    );
  }
